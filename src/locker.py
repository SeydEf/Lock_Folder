...
def lock(fpath, password):
    ...
    command1 = 'ren "{}" "Control Panel.{{21EC2020-3AEA-1069-A2DD-{}}}"'.format(fname, key)
    command2 = 'attrib +h +s "Control Panel.{{21EC2020-3AEA-1069-A2DD-{}}}"'.format(key)
...
def unlock(fpath, password, key):
    fname = os.path.basename(fpath)
    cwd = '/'.join(fpath.split('/')[:-1]) + '/'
    command1 = 'attrib -h -s "Control Panel.{{21EC2020-3AEA-1069-A2DD-{}}}"'.format(key)
    command2 = 'ren "Control Panel.{{21EC2020-3AEA-1069-A2DD-{}}}" "{}"'.format(key, fname)
